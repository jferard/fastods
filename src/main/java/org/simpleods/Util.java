...
public class Util {
	...
	public boolean writeStringArray(final ZipOutputStream o, final String[] sText) {

		try {
			for (int n = 0; n < sText.length; n++) {
				o.write(sText[n].getBytes(), 0, sText[n].length());
			}
		} catch (Exception e) {
			e.printStackTrace();
			return false;
	...
	public boolean writeString(final ZipOutputStream o, final String sText) {
		...
		try {
			o.write(sText.getBytes(), 0, sText.length());
		} catch (Exception e) {
			e.printStackTrace();
			return false;
		}
	...
	public String createHexColor(int nRed, int nGreen, int nBlue) {
		...
		return ("#" + toHexString(nRed) + toHexString(nGreen) + toHexString(nBlue));
	 ...
	 * @param sHaystack	The source string to be checked
	 * @param sNeedle	The sub-string to be replaced
	 * @param sReplacement	The replacement string
	 * @return	The new string
	 */
	public String replace(final String sHaystack, final String sNeedle, final String sReplacement) {
		int nStart = 0;
		int nEnd = 0;
	...
	public String toXmlString(String sString) {
		...
		for (int n = 0; n < sString.length(); n++) {
			...
			if (c > 128) {
				sString = replace(sString, sString.substring(n, n + 1), "&#" + c + ";");
			}
		}
	...
	/**
	 * Get the todays date in format sDateFormat. See Javas 'SimpleDateFormat' for details.<br>
	 * The following pattern letters are defined (all other characters from 'A' to 'Z' and from 'a' to 'z' are reserved):<br>
	 ...
	 * @param sDateFormat The date format for todays date that should be returned.
	 ...
	 * @return The row, e.g. A1 will return 0, B1 will return 1, E1 will return 4
	 */
	public int positionToColumn(final String sPos) {
		...
		for (int n = s.length() - 1; n >= 0; n--) {
			...
			//------------------------------------------------
			if (c >= 'A' && c <= 'Z') {
				nValue += (c - 64) * (int) Math.pow((double) 26, (double) nCount);
				nCount++;
				if (nCount >= 2) {
					break; // 'IV' is the last column name, so break out after the second letter
				}	
			}
		}
	 ...
	 * @param sPos
	 *            The cell position in the range 'A1' to 'IV65536'
	 * @return The column, e.g. A1 will return 0, B3 will return 2, X65000 will return 64999
	 */
	public int positionToRow(final String sPos) {
		...
		for (int n = sPos.length() - 1; n >= 0; n--) {
			...
			// -------------------------------------------------
			if (c >= '0' && c <= '9') {
				nValue += (c - 48) * (int) Math.pow((double) 10, (double) nCount);
				nCount++;
				if (nCount >= 5) {
					break; // 65536 is the highest row, so break out after the fifth number
				}
	...
	public void copyStylesFromTo(final OdsFile source, final OdsFile target) {
		...
		target.getContent().setTableStyles(source.getContent().getTableStyles());
	 ...
	 */
	public void appendElement(final StringBuffer sb, final String sElementName, final String sValue) {
	 ...
	 *            The value of the element
	 */
	public void appendElement(final StringBuffer sb, final String sElementName, final int nValue) {
		sb.append(" " + sElementName + "=\"");
