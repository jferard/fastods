...
public class StylesEntryTest {
	...
	public final void testStyleTagCreateTwice() {
		Assert.assertTrue(this.stylesContainer.addStyle(this.st1,
				Dest.STYLES_COMMON_STYLES, Mode.CREATE));
		Assert.assertFalse(this.stylesContainer.addStyle(this.st2,
				Dest.STYLES_COMMON_STYLES, Mode.CREATE));
		final Map<String, StyleTag> styleTagByName = this.stylesContainer
				.getStyleTagByName(Dest.STYLES_COMMON_STYLES);
		Assert.assertEquals(1, styleTagByName.size());
		Assert.assertEquals(this.st1, styleTagByName.get("table-cell@a"));
	}
	...
	public final void testStyleTagCreateThenUpdate() {
		Assert.assertTrue(this.stylesContainer.addStyle(this.st1,
				Dest.STYLES_COMMON_STYLES, Mode.CREATE));
		Assert.assertTrue(this.stylesContainer.addStyle(this.st2,
				Dest.STYLES_COMMON_STYLES, Mode.UPDATE));
		final Map<String, StyleTag> styleTagByName = this.stylesContainer
				.getStyleTagByName(Dest.STYLES_COMMON_STYLES);
		Assert.assertEquals(1, styleTagByName.size());
		Assert.assertEquals(this.st2, styleTagByName.get("table-cell@a"));
	}

	@Test
	public final void testStyleTagUpdate() {
		Assert.assertFalse(
				this.stylesContainer.addStyle(this.st2, Dest.STYLES_COMMON_STYLES, Mode.UPDATE));
		final Map<String, StyleTag> styleTagByName = this.stylesContainer
				.getStyleTagByName(Dest.STYLES_COMMON_STYLES);
		Assert.assertEquals(0, styleTagByName.size());
	}

	@Test
	public final void testStyleTagCreateThenUpdateIfExists() {
		Assert.assertTrue(this.stylesContainer.addStyle(this.st1, Dest.STYLES_COMMON_STYLES, Mode.CREATE));
		Assert.assertTrue(
				this.stylesContainer.addStyle(this.st2, Dest.STYLES_COMMON_STYLES, Mode.CREATE_OR_UPDATE));
		final Map<String, StyleTag> styleTagByName = this.stylesContainer
				.getStyleTagByName(Dest.STYLES_COMMON_STYLES);
		Assert.assertEquals(1, styleTagByName.size());
		Assert.assertEquals(this.st2, styleTagByName.get("table-cell@a"));
	...
	public final void testDataStyleCreateTwice() {
		...
		Assert.assertFalse(
				this.stylesEntry.addDataStyle(this.ds2, Mode.CREATE));
	...
	public final void testDataStyleUpdate() {
		Assert.assertFalse(
				this.stylesEntry.addDataStyle(this.ds2, Mode.UPDATE));
		Map<String, DataStyle> dataStyles = this.stylesEntry.getDataStyles();
	...
	public final void testDataStyleCreateThenUpdateIfExists() {
		Assert.assertTrue(this.stylesEntry.addDataStyle(this.ds1, Mode.CREATE));
		Assert.assertTrue(
				this.stylesEntry.addDataStyle(this.ds2, Mode.CREATE_OR_UPDATE));
		Map<String, DataStyle> dataStyles = this.stylesEntry.getDataStyles();
		Assert.assertEquals(1, dataStyles.size());
		Assert.assertEquals(this.ds2, dataStyles.get("a"));
	}
	...
	public final void testPageStyleCreateTwice() {
		Assert.assertTrue(
				this.stylesEntry.addMasterPageStyle(this.ps1, Mode.CREATE));
		Assert.assertFalse(
				this.stylesEntry.addMasterPageStyle(this.ps2, Mode.CREATE));
		Map<String, MasterPageStyle> masterPageStyles = this.stylesEntry
				.getPageStyles();
	...
	public final void testPageStyleCreateThenUpdate() {
		Assert.assertTrue(
				this.stylesEntry.addMasterPageStyle(this.ps1, Mode.CREATE));
		Assert.assertTrue(
				this.stylesEntry.addMasterPageStyle(this.ps2, Mode.UPDATE));
		Map<String, MasterPageStyle> masterPageStyles = this.stylesEntry
				.getPageStyles();
		Assert.assertEquals(1, masterPageStyles.size());
	...
	public final void testPageStyleCreateThenUpdateIfExists() {
		Assert.assertTrue(
				this.stylesEntry.addMasterPageStyle(this.ps1, Mode.CREATE));
		Assert.assertTrue(this.stylesEntry.addMasterPageStyle(this.ps2,
