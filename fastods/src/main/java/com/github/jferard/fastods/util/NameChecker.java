...
public class NameChecker {
    public String checkStyleName(final String name) {
        if (name == null || name.isEmpty()) throw new IllegalArgumentException(String.format("Bad name: %s", name));
    ...
    public boolean isNCNameStartChar(final int codePoint) {
        return 'a' <= codePoint && codePoint <= 'z' || 'A' <= codePoint && codePoint <= 'Z' || codePoint == '-' ||
                0xc0 <= codePoint && codePoint <= 0x2ff && codePoint != 0xd7 && codePoint != 0xf7 || 0x370 <=
                codePoint && codePoint <= 0x1fff && codePoint != 0x37e || codePoint == 0x200c || codePoint == 0x200d
                || 0x2070 <= codePoint && codePoint <= 0x218f || 0x2c00 <= codePoint && codePoint <= 0x2fef || 0x3001
                <= codePoint && codePoint <= 0xd7ff || 0xf900 <= codePoint && codePoint <= 0xfdcf || 0xfdf0 <=
    ...
    public boolean isNCNameChar(final int codePoint) {
        return 'a' <= codePoint && codePoint <= 'z' || 'A' <= codePoint && codePoint <= 'Z' || codePoint == '_' ||
                codePoint == '-' || codePoint == '.' || '0' <= codePoint && codePoint <= '9' || codePoint == 0xb7 ||
                0xc0 <= codePoint && codePoint <= 0x2ff && codePoint != 0xd7 && codePoint != 0xf7 || 0x300 <=
                codePoint && codePoint <= 0x36F || 0x370 <= codePoint && codePoint <= 0x1fff && codePoint != 0x37e ||
                codePoint == 0x200c || codePoint == 0x200d || 0x203F <= codePoint && codePoint <= 0x2040 || 0x2070 <=
                codePoint && codePoint <= 0x218f || 0x2c00 <= codePoint && codePoint <= 0x2fef || 0x3001 <= codePoint
                && codePoint <= 0xd7ff || 0xf900 <= codePoint && codePoint <= 0xfdcf || 0xfdf0 <= codePoint &&
                codePoint <= 0xfffd || 0x10000 <= codePoint && codePoint <= 0xeffff;
    }
}
